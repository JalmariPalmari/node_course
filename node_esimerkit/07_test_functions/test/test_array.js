...
describe('Using String#split', ()=>{
  ...
  it('should be the same array', ()=>{
    assert.equal(expectedArr.length, arr.length, 'Arrays have equal length')
    for (let i=0; i<expectedArr.length; i++) {
      assert.equal(expectedArr[i], arr[i], i + 'th elements are equals')
    }
  })

  it('should be the same array II', ()=>{
    const arr2 = 'a,b,c,d'.split(',')  // not using beforeEach method
    assert.equal(expectedArr.length, arr2.length, 'Arrays have equal length')
