...
describe('String#splitToArray', () => {
  it('should return the same array', () => {
  	//list.reverse();
    assert.equal(expectedArr.length, list.length, 'arrays have same length')
    for (let i = 0; i < expectedArr.length; i++) {
      assert.equal(expectedArr[i], list[i], i + 'th elements are equal')
    }
  })
})  
...
describe('Array#reverseArray', () => {
  it('should return the reversed array', () => {
    ...
    assert.equal(reversedArr.length, reversed.length, 'arrays have same length')
    for (let i = 0; i < reversedArr.length; i++) {
      assert.equal(reversedArr[i], reversed[i], i + 'th elements are equal')
    }
  })
})  
